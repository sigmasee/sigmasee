...
func (n *ApexCustomer) IsNode() {}

// IsNode implements the Node interface check for GQLGen.
func (n *ApexCustomerIdentity) IsNode() {}
...
func (c *Client) noder(ctx context.Context, table string, id string) (Noder, error) {
	switch table {
	case apexcustomer.Table:
		query := c.ApexCustomer.Query().
			Where(apexcustomer.ID(id))
		query, err := query.CollectFields(ctx, "ApexCustomer")
		if err != nil {
			return nil, err
		}
		...
		return n, nil
	case apexcustomeridentity.Table:
		query := c.ApexCustomerIdentity.Query().
			Where(apexcustomeridentity.ID(id))
		query, err := query.CollectFields(ctx, "ApexCustomerIdentity")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
