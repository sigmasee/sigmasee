...
func (iq *IdentityQuery) QueryCustomer() *CustomerQuery {
...
func (iq *IdentityQuery) WithCustomer(opts ...func(*CustomerQuery)) *IdentityQuery {
	query := (&CustomerClient{config: iq.config}).Query()
...
func (iq *IdentityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Identity, error) {
	...
	_spec.Node.Schema = iq.schemaConfig.Identity
	...
	if query := iq.withCustomer; query != nil {
		if err := iq.loadCustomer(ctx, query, nodes, nil,
			func(n *Identity, e *Customer) { n.Edges.Customer = e }); err != nil {
			return nil, err
		}
	}
...
func (iq *IdentityQuery) loadCustomer(ctx context.Context, query *CustomerQuery, nodes []*Identity, init func(*Identity), assign func(*Identity, *Customer)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Identity)
	for i := range nodes {
		if nodes[i].customer_identities == nil {
			continue
		}
		fk := *nodes[i].customer_identities
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "customer_identities" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (iq *IdentityQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(iq.schemaConfig.Identity)
	ctx = internal.NewSchemaConfigContext(ctx, iq.schemaConfig)
...
func (iq *IdentityQuery) ForUpdate(opts ...sql.LockOption) *IdentityQuery {
...
func (iq *IdentityQuery) ForShare(opts ...sql.LockOption) *IdentityQuery {
	if iq.driver.Dialect() == dialect.Postgres {
		iq.Unique(false)
	}
	iq.modifiers = append(iq.modifiers, func(s *sql.Selector) {
...
func (iq *IdentityQuery) Modify(modifiers ...func(s *sql.Selector)) *IdentitySelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (is *IdentitySelect) Modify(modifiers ...func(s *sql.Selector)) *IdentitySelect {
