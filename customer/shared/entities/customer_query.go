...
type CustomerQuery struct {
	...
	withCustomerSettings *CustomerSettingQuery
...
func (cq *CustomerQuery) QueryIdentities() *IdentityQuery {
	query := (&IdentityClient{config: cq.config}).Query()
...
// QueryCustomerSettings chains the current query on the "customer_settings" edge.
func (cq *CustomerQuery) QueryCustomerSettings() *CustomerSettingQuery {
	query := (&CustomerSettingClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CustomerQuery) Clone() *CustomerQuery {
	...
	return &CustomerQuery{
		...
		withIdentities:       cq.withIdentities.Clone(),
		withCustomerSettings: cq.withCustomerSettings.Clone(),
...
func (cq *CustomerQuery) WithIdentities(opts ...func(*IdentityQuery)) *CustomerQuery {
	query := (&IdentityClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withIdentities = query
	return cq
}

// WithCustomerSettings tells the query-builder to eager-load the nodes that are connected to
// the "customer_settings" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CustomerQuery) WithCustomerSettings(opts ...func(*CustomerSettingQuery)) *CustomerQuery {
	query := (&CustomerSettingClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withCustomerSettings = query
	return cq
}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Customer, error) {
	var (
		...
		_spec       = cq.querySpec()
		loadedTypes = [2]bool{
			cq.withIdentities != nil,
			cq.withCustomerSettings != nil,
		}
	)
	...
	_spec.Node.Schema = cq.schemaConfig.Customer
	...
	if query := cq.withIdentities; query != nil {
		if err := cq.loadIdentities(ctx, query, nodes,
			func(n *Customer) { n.Edges.Identities = []*Identity{} },
			func(n *Customer, e *Identity) { n.Edges.Identities = append(n.Edges.Identities, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withCustomerSettings; query != nil {
		if err := cq.loadCustomerSettings(ctx, query, nodes, nil,
			func(n *Customer, e *CustomerSetting) { n.Edges.CustomerSettings = e }); err != nil {
			return nil, err
		}
	}
	for name, query := range cq.withNamedIdentities {
		if err := cq.loadIdentities(ctx, query, nodes,
			func(n *Customer) { n.appendNamedIdentities(name) },
			func(n *Customer, e *Identity) { n.appendNamedIdentities(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CustomerQuery) loadIdentities(ctx context.Context, query *IdentityQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *Identity)) error {
	...
	query.Where(predicate.Identity(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(customer.IdentitiesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.customer_identities
		if fk == nil {
			return fmt.Errorf(`foreign-key "customer_identities" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "customer_identities" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (cq *CustomerQuery) loadCustomerSettings(ctx context.Context, query *CustomerSettingQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *CustomerSetting)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Customer)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.CustomerSetting(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(customer.CustomerSettingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.customer_customer_settings
		if fk == nil {
			return fmt.Errorf(`foreign-key "customer_customer_settings" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "customer_customer_settings" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (cq *CustomerQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(cq.schemaConfig.Customer)
	ctx = internal.NewSchemaConfigContext(ctx, cq.schemaConfig)
...
func (cq *CustomerQuery) WithNamedIdentities(name string, opts ...func(*IdentityQuery)) *CustomerQuery {
	query := (&IdentityClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if cq.withNamedIdentities == nil {
		cq.withNamedIdentities = make(map[string]*IdentityQuery)
	}
	cq.withNamedIdentities[name] = query
	return cq
}
