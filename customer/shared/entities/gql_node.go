...
// IsNode implements the Node interface check for GQLGen.
func (n *CustomerOutbox) IsNode() {}

// IsNode implements the Node interface check for GQLGen.
func (n *CustomerSetting) IsNode() {}
...
func (c *Client) noder(ctx context.Context, table string, id string) (Noder, error) {
	...
	case customer.Table:
		...
		return n, nil
	case customeroutbox.Table:
		query := c.CustomerOutbox.Query().
			Where(customeroutbox.ID(id))
		query, err := query.CollectFields(ctx, "CustomerOutbox")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
		...
		return n, nil
	case customersetting.Table:
		query := c.CustomerSetting.Query().
			Where(customersetting.ID(id))
		query, err := query.CollectFields(ctx, "CustomerSetting")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []string) ([]Noder, error) {
	...
	case customersetting.Table:
		query := c.CustomerSetting.Query().
			Where(customersetting.IDIn(ids...))
		query, err := query.CollectFields(ctx, "CustomerSetting")
		if err != nil {
			return nil, err
		}
